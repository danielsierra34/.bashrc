...
from graphify.extractors.generic_text import extract_generic_text  # noqa: F401
...
def _get_extractor(path: Path) -> Any | None:
    ...
    if not suffix:
        ...
        return None
    specific = _DISPATCH.get(suffix)
    if specific is not None:
        return specific
    # Generic textual fallback, mirrors detect.classify_file's final branch:
    # only for an extension absent from every known category (so .yaml/.txt/
    # any other already-categorized-but-extractor-less extension is completely
    # unaffected) AND that a binary/text sniff confirms is textual.
    from graphify.detect import _is_unrecognized_extension, _looks_like_text
    if _is_unrecognized_extension(path) and _looks_like_text(path):
        return extract_generic_text
    return None
