...
# Every extension classify_file() special-cases above. Used only to scope the
# generic textual fallback: an extension already in one of these sets keeps
# its EXACT existing behavior, no matter whether it has a dedicated AST
# extractor -- the fallback only ever fires for an extension absent from all
# of them.
_KNOWN_EXTENSIONS = (
...
# Bytes a genuine text file is made of: printable ASCII/Latin-1/UTF-8
# continuation bytes (>= 0x20) plus tab/LF/CR. What's excluded is the
# NUL/low-control range (0x00-0x08, 0x0B, 0x0C, 0x0E-0x1F) that shows up
# constantly in binary formats (zip/gzip headers, compiled objects, images)
# and essentially never in text.
_TEXT_BYTES = frozenset(range(0x20, 0x100)) | {0x09, 0x0A, 0x0D}

# Below this fraction of "text-like" bytes in the sample, treat the file as
# binary. Deliberately generous (some legitimate text -- data tables, box
# drawing) dips below 100%; the NUL-byte short-circuit in _looks_like_text
# already catches the common binary formats outright.
_TEXT_BYTE_RATIO_MIN = 0.85


def _looks_like_text(path: Path, sample_size: int = 8192) -> bool:
    """Binary/text sniff independent of extension.

    Feeds the generic textual fallback (classify_file's final branch, and
    extract._get_extractor's matching fallback): an extension graphify has no
    category for is not by itself proof the file is noise -- only this
    decides that. Mirrors the heuristic git/diff use for the same purpose: a
    NUL byte in a leading sample is treated as decisive proof of binary
    content, since no ordinary text encoding embeds one. Failing that, the
    sample must still be mostly printable/whitespace bytes -- a coarse,
    dependency-free stand-in for `file --mime-type`.
    """
    try:
        with path.open("rb") as f:
        ...
        return False
    if not sample:
        return True  # empty file: nothing in it proves binary
    if b"\x00" in sample:
        return False
    text_like = sum(1 for b in sample if b in _TEXT_BYTES)
    return (text_like / len(sample)) >= _TEXT_BYTE_RATIO_MIN


def _is_unrecognized_extension(path: Path) -> bool:
    """True when *path* has a non-empty extension outside every category
    classify_file() special-cases (CODE/DOC/PAPER/IMAGE/OFFICE/VIDEO/Google
    Workspace).

    Used by extract._get_extractor to scope the generic textual fallback to
    exactly the extensions classify_file() would otherwise drop as
    unclassified -- it must never fire for an extension already handled, even
    one with no AST extractor of its own (e.g. .yaml/.txt stay exactly as
    they are: DOCUMENT, no AST quick-scan, semantic-pass only). Extensionless
    files are never "unrecognized" here; classify_file() routes those through
    the shebang path instead, which this fallback does not touch.
    """
...
def classify_file(path: Path) -> FileType | None:
    ...
    # Generic textual fallback: an extension not in any category above is not
    # proof the file is noise -- only the binary/text sniff decides that. A
    # textual file with an unrecognized extension is classified as CODE so it
    # flows through the same no-LLM `graphify update` path as any other
    # source file; extract._get_extractor registers extract_generic_text for
    # it when no dedicated extractor claims the extension. Binaries and every
    # already-categorized extension are unaffected: _is_unrecognized_extension
    # only returns True for an extension absent from every set above, and a
    # binary always fails _looks_like_text regardless of extension.
    if _is_unrecognized_extension(path) and _looks_like_text(path):
        return FileType.CODE
    return None
